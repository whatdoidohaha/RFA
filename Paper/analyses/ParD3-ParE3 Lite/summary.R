...
F90_lower <- min(which(sparsity > 0.9 * cv_second[2L])) / prod(q_list)
...
S90_lower <- interpolate(frac, apply(pred, 2L, mean, na.rm = TRUE), 0.9 * cv_second[2L])
