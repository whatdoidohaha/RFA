...
# Color scheme: intercept = black, first-order = magenta, second-order = cyan, third-order = yellow
ord_col <- c('#000000', '#ec008b', '#00adef', '#fff100')

n_terms <- 30L
plot(1L:n_terms, sparsity[1L:n_terms], type = 'l', ylim = c(0, 1))
points(1L:n_terms, sparsity[1L:n_terms], cex = 2)
points(1L:n_terms, sparsity[1L:n_terms], pch = 16L, cex = 2,
       col = ord_col[e_ord[ord_by_contribution[1L:n_terms]] + 1L])
abline(h = 0.9)
abline(v = F90_upper * prod(q_list))
