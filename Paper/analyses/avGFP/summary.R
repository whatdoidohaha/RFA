...
# Plotting the first and second-order terms.

e1 <- matrix(M2$e[2L:(n + 1L)], nrow = 1L)
e2 <- matrix(NA_real_, nrow = n, ncol = n)
e2[lower.tri(e2, diag = FALSE)] <- M2$e[-c(1:(n + 1L))]
pal1 <- colorRampPalette(c('#04a54d', '#ffffff'))
pal2 <- colorRampPalette(c('#ffffff', '#2d378c'))

gplots::heatmap.2(rbind(e1, e2), Rowv = FALSE, Colv = FALSE, dendrogram = 'none', trace = 'none',
                  breaks = c(-4, seq(-2.3, 2.3, length.out = 19L), 4),
                  col = c(pal1(10), pal2(10)))
