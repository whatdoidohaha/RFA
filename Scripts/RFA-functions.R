...
# effects and nonspecific epistasis. See the workflow RFA for more information.
#
...
import_data <- function(dir_genotypes, dir_phenotypes) {
  ...
  if(any(reducible)) {
    ...
    genotypes <<- vapply(1L:ncol(genotypes),
...
infer_model <- function(genotypes, y, site_combn, lambda = 0, init_model = NULL,
  ...
  if(!is.null(init_model)) {
    ...
    if(nonspec_type == 'none') {
      ...
      # Initial value for the sigmoid is the data range.
...
cross_validation <- function(genotypes, y, site_combn, k, lambda_list, niter,
  ...
  # plot_CV : logical
  #   If TRUE, plots the observed phenotype versus prediction for both in-sample
  #   and out-of-sample analysis.
  
  if(plot_CV) par(mfrow = c(1, 2))
    ...
    for(j in seq_along(lambda_list)) { # j indexes lasso penalties
      ...
      for(i in 1L:k) { # i indexes genotype partitions
        ...
        model <- infer_model(genotypes[membership != i, ], y[membership != i],
                             ...
                             silent = 1, epsilon = epsilon)
        
        # Predicted phenotype.
        p <- apply_nonspecific_epistasis(as.vector(X %*% model$e), model$nonspec_param)
        
        # In-sample and out-of-sample R2.
        IR2 <- 1 - sum((y[membership != i] - p[membership != i]) ^ 2) /
          sum((y[membership != i] - mean(y[membership != i])) ^ 2)
        
        OR2 <- 1 - sum((y[membership == i] - p[membership == i]) ^ 2) /
          sum((y[membership == i] - mean(y[membership == i])) ^ 2)
        
        # Out-of-sample R2.
        cvR2[k * (iter - 1L) + i, j] <- OR2
        
        cat('CV iteration ', iter, ', lambda = ', lambda_list[j], ', partition ',
            i, '; ', 'Out-of-sample R2 = ', round(OR2, 3), ', in-sample R2 = ',
            round(IR2, 3), '\n', sep = '')
        
        if(i == 1L & plot_CV) { # Plot only for the first genotype partition
          
          plot(p[membership == i], y[membership == i], xlab = 'Predicted', ylab = 'Observed',
               main = paste('Out-of-sample; lambda =', round(lambda_list[j], 3)),
               cex = 0.5, col = rgb(0, 0, 0, 0.5), pch = 16)
          abline(a = 0, b = 1)
          
          plot(p[membership != i], y[membership != i], xlab = 'Predicted', ylab = 'Observed',
               main = paste('In-sample; lambda =', round(lambda_list[j], 3)),
               cex = 0.5, col = rgb(0, 0, 0, 0.5), pch = 16)
          abline(a = 0, b = 1)
        }
      }
...
cv_across_replicates <- function(genotypes, replicates, site_combn, lambda_list,
  ...
  # genotypes, site_combn, lambda_list, epsilon, plot_CV :
  ...
  for(i in 1L:ncol(replicates)) { # i indexes replicates
    ...
    y_minus_i <- apply(replicates[, -i, drop = FALSE], 1, mean, na.rm = TRUE)
    
    for(j in seq_along(lambda_list)) { # j indexes lasso penalties
      
      model <- infer_model(genotypes, y_minus_i, site_combn, lambda_list[j],
                           norm_e = FALSE, silent = 1, epsilon = epsilon)
      
      # Predicted phenotype.
      p <- apply_nonspecific_epistasis(as.vector(X %*% model$e), model$nonspec_param)
      
      # In-sample and out-of-sample R2.
      IR2 <- 1 - sum((y_minus_i - p) ^ 2) / sum((y_minus_i - mean(y_minus_i)) ^ 2)
      
      OR2 <- 1 - sum((replicates[, i] - p) ^ 2) /
        sum((replicates[, i] - mean(replicates[, i])) ^ 2)
      
      cvR2[i, j] <- OR2
      
      cat('CV leaving out replicate ', i, ', lambda = ', lambda_list[j],
          '; Out-of-sample R2 = ', round(OR2, 3), ', in-sample R2 = ',
          round(IR2, 3), '\n', sep = '')
      
      if(i == 1L & plot_CV) { # Plot only for the first partition
        
        plot(p, replicates[, i], xlab = 'Predicted', ylab = 'Observed',
             main = paste('Out-of-sample; lambda =', round(lambda_list[j], 3)),
             cex = 0.5, col = rgb(0, 0, 0, 0.5), pch = 16)
        abline(a = 0, b = 1)
        
        plot(p, y_minus_i, xlab = 'Predicted', ylab = 'Observed',
             main = paste('In-sample; lambda =', round(lambda_list[j], 3)),
             cex = 0.5, col = rgb(0, 0, 0, 0.5), pch = 16)
        abline(a = 0, b = 1)
      }
    ...
    for(i in 1L:k) { # i indexes genotype partitions
      ...
      for(j in seq_along(lambda_list)) { # j indexes lasso penalties
        ...
        res <- lbfgs(objective, gradient, init_param,
                     ...
                     invisible = 1, epsilon = epsilon)$par
